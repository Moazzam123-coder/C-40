class Game {
  ...
  end()
  {
    ...
    e1.html(player.name+" HAVE REACHED THE END & YOUR RANK IS:"+player.rank);
    e1.position(displayWidth/3,100);
    console.log(Player.rank);
    //image(car1_img,displayWidth/2,displayHeight/2);
    for(var i = 1;i<=4;i++){
      if(player.rank===i){
        cars[i-1].visible=true;
      }
      else{
        cars[i-1].visible=false;
      }
    }
    drawSprites();
    console.log("GAME ENDED");
    
    //text("YOUR RANK IS:"+player.rank,displayWidth/2,50);
  }
}
